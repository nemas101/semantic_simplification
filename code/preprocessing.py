...
def delete_beginning(text):
    beginning_text_index = text.index([i for i in text if re.findall(r'\*\*\* START OF THE PROJECT GUTENBERG', i)][0])
    text_cleared_beg = text[beginning_text_index+1:]
    return text_cleared_beg

def delete_end(text):
    end_text_index = text.index([i for i in text if re.findall(r'\*\*\* END OF THE PROJECT GUTENBERG', i)][0])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--text', required=True, help="Name of the text in the texts_folder")
